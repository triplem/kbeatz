...
class RepairOnStartupTest {
    ...
    @Suppress("MagicNumber") // default 60s timeout
    private fun buildService(root: Path, repairTimeoutSeconds: Long = 60L): LibraryScanService =
        LibraryScanService(
            libraryRoot = root,
            walker = walker,
    ...
    @Test
    fun `repairOnStartup does nothing when no lock files exist`(@TempDir root: Path) = runTest {
        val svc = buildService(root)
    ...
    @Test
    fun `repairOnStartup re-indexes and deletes lock file`(@TempDir root: Path) = runTest {
        val svc = buildService(root)
    ...
    @Test
    fun `repairOnStartup handles multiple lock files in different directories`(@TempDir root: Path) = runTest {
        val svc = buildService(root)
    ...
    @Test
    fun `isRepairComplete returns false before repairOnStartup is called`(@TempDir root: Path) = runTest {
        val svc = buildService(root)
        assertFalse(svc.isRepairComplete(), "Repair should not be complete before repairOnStartup runs")
    }

    @Test
    fun `isRepairComplete returns true after repairOnStartup when no lock files exist`(@TempDir root: Path) = runTest {
        val svc = buildService(root)
        svc.repairOnStartup()
    ...
    @Test
    fun `repairOnStartup marks repair complete after timeout when saveAll is slow`(@TempDir root: Path) = runTest {
        @Suppress("MagicNumber") // 1s timeout to trigger the repair-timeout code path
        val svc = buildService(root, repairTimeoutSeconds = 1L)
