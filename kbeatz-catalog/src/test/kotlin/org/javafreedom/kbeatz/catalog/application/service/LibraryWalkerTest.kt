...
class LibraryWalkerTest {
    ...
    @Test
    fun `walk groups all FLAC files in one directory into one AlbumGroup`(@TempDir root: Path) {
    ...
    @Test
    fun `walk merges disc1 and disc2 subdirectories into a single AlbumGroup`(@TempDir root: Path) {
    ...
    @Test
    fun `walk returns two AlbumGroups for two unrelated album directories`(@TempDir root: Path) {
    ...
    @Test
    fun `walk groups correctly when DATE tag is absent`(@TempDir root: Path) {
    ...
    @Test
    fun `walk includes only FLAC files and ignores other file types`(@TempDir root: Path) {
    ...
    @Test
    fun `walk handles empty library root with no FLAC files`(@TempDir root: Path) {
    ...
    @Test
    fun `walk skips unreadable FLAC files and continues`(@TempDir root: Path) {
    ...
    @Test
    fun `walk merges same album in two separate directories into one group`(@TempDir root: Path) {
    ...
    @Test
    fun `walk merges tracks with same year but different DATE formats into one group`(@TempDir root: Path) {
    ...
    @Test
    fun `walk rootPath for merged group is the shallowest directory`(@TempDir root: Path) {
    ...
    @Test
    fun `walk sourceDirs contains single directory for albums from one source`(@TempDir root: Path) {
