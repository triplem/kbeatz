...
object PathGuard {
    ...
    /**
     * Asserts that [path] is within [libraryRoot], throwing [PathTraversalException] otherwise.
     *
     ...
     * @throws PathTraversalException if [path] resolves to a location outside [libraryRoot].
     */
    fun assertWithinLibraryRoot(path: Path, libraryRoot: Path) {
        ...
        if (!resolved.startsWith(normalizedRoot)) {
            ...
            throw PathTraversalException("Path is outside the library root: $path")
        }
    }
}
