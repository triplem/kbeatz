package org.javafreedom.kbeatz.tagger.command
    ...
    override fun run() {
        ...
        for (dir in targets) {
            ...
            if (dryRun) {
                ...
                // TODO: fetch from kbeatz-metadata-discogs, map to FlacFile, write tags
...
// Converts a CLI string argument to a kotlinx.io Path with existence validation.
private fun String.toKtxPath(): Path = Path(this).also {
    require(SystemFileSystem.exists(it)) { "Path does not exist: $this" }
}

// Recursive directory walk using SystemFileSystem.
