...
function serverPage(content: Album[], pageNum: number, totalElements: number): AlbumPage {
  return { content, page: pageNum, size: 48, totalElements, totalPages: Math.ceil(totalElements / 48) }
}
...
describe('toServerParams', () => {
  it('maps page/size and omits empty filters', () => {
    expect(toServerParams({ page: 2, size: 48, filters: EMPTY_FILTERS })).toEqual({ page: 2, size: 48 })
  })
  ...
  it('maps the first selected value of each multi-select axis and free-text query', () => {
    ...
    expect(toServerParams({ page: 0, size: 24, filters })).toEqual({
      page: 0,
      size: 24,
...
describe('useAlbumPage', () => {
  it('fetches exactly one server page when enabled', async () => {
    ...
    const { result } = renderHook(
      () => useAlbumPage({ page: 1, size: 48, filters: EMPTY_FILTERS }, true),
      { wrapper },
    )
    ...
    expect(mockListAlbums).toHaveBeenCalledWith({ page: 1, size: 48 })
  })

  it('does not fetch when disabled (client-side mode)', () => {
    renderHook(() => useAlbumPage({ page: 0, size: 48, filters: EMPTY_FILTERS }, false), { wrapper })
  ...
  it('sends mapped filter params to the server', async () => {
    ...
    const { result } = renderHook(() => useAlbumPage({ page: 0, size: 48, filters }, true), { wrapper })
    await waitFor(() => expect(result.current.isSuccess).toBe(true))
