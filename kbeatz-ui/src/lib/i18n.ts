...
/** Update the <html lang> attribute to reflect the active locale. */
function syncLangAttr(lng: string) {
  if (typeof document !== 'undefined') {
    document.documentElement.lang = lng.split('-')[0] ?? lng
  }
}

// Sync immediately after init and on every subsequent change.
i18n.on('initialized', () => {
  syncLangAttr(i18n.language)
})
